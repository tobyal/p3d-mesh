...
from models.psp.networks.extra_modules import HierarchicalContextPropagationCtx
...
class PSP_PointTransformer(nn.Module):
    def __init__(self, cfgs, input_dim=3):
        super().__init__()
        self.stem = nn.Sequential(
            nn.Conv1d(input_dim, cfgs.encoder_dim, 1),
        ...
        ctx_dim = getattr(cfgs, "context_dim", cfgs.encoder_dim)
        ratio1 = getattr(cfgs, "context_ratio1", 0.25)
        ratio2 = getattr(cfgs, "context_ratio2", 0.25)
        k_down = getattr(cfgs, "context_k_down", cfgs.k)
        k_ctx = getattr(cfgs, "context_k_ctx", cfgs.k)

        self.context_propagation = HierarchicalContextPropagationCtx(
            in_channels=cfgs.encoder_dim,
            ctx_channels=ctx_dim,
            ratio1=ratio1,
            ratio2=ratio2,
            k_down=k_down,
            k_ctx=k_ctx,
            use_bn=cfgs.encoder_bn,
        )
        context_out_dim = ctx_dim
        ...
        if cfgs.global_mlp:
            ...
            fuse_in_dim = cfgs.encoder_dim * 4 + 128 + context_out_dim
            
            self.fuse_mlp = nn.Sequential(
                nn.Conv1d(fuse_in_dim, cfgs.out_dim*2, 1),
                nn.BatchNorm1d(cfgs.out_dim*2),
                nn.ReLU(inplace=True),
                nn.Conv1d(cfgs.out_dim*2, cfgs.out_dim, 1),
                nn.BatchNorm1d(cfgs.out_dim),
            ...
            self.global_mlp = nn.Identity()
            fuse_in_dim = cfgs.encoder_dim * 5 + context_out_dim
            self.fuse_mlp = nn.Sequential(
                    nn.Conv1d(fuse_in_dim, cfgs.out_dim, 1),
                    nn.BatchNorm1d(cfgs.out_dim),
                    nn.ReLU(inplace=True)
                )
    ...
    def forward(self, pts, feat=None):
        ...
        fuse_list = [feat0, feat1, feat2, feat3, global_feat]
        ctx_feat = self.context_propagation(pts, feat0)
        fuse_list.append(ctx_feat)
        out = self.fuse_mlp(torch.cat(fuse_list, dim=1))
