...
from models.psp.utils.kernel_utils import load_kernels
...
class PatchContextAggregator(nn.Module):
    def __init__(self, feat_dim, patch_dim=None, k=8, use_radius=True):
        super().__init__()
        self.feat_dim = feat_dim
        self.patch_dim = feat_dim if patch_dim is None else patch_dim
        self.k = k
        self.use_radius = use_radius

        token_in_dim = feat_dim * 2 + (1 if use_radius else 0)

        self.token_mlp = nn.Sequential(
            nn.Linear(token_in_dim, self.patch_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.patch_dim, self.patch_dim),
            nn.ReLU(inplace=True)
        )

        # [center_token, neighbor_token, rel_xyz, dist]
        self.attn_mlp = nn.Sequential(
            nn.Linear(self.patch_dim * 2 + 4, self.patch_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.patch_dim, 1)
        )
    ...
    def _knn_indices(x, k):
        P = x.shape[0]
        if P == 1:
            return torch.zeros((1, 0), dtype=torch.long, device=x.device)

        k_eff = min(k + 1, P)  # 先取自己在内的 k+1 个邻居
        x_batched = x.transpose(0, 1).unsqueeze(0).contiguous()   # (1, 3, P)

        _, knn_idx = get_knn_pts(
            k=k_eff,
            pts=x_batched,
            center_pts=x_batched,
            return_idx=True
        )   # (1, P, k_eff)

        knn_idx = knn_idx[0]   # (P, k_eff)

        self_mask = knn_idx != torch.arange(P, device=x.device).unsqueeze(1)
        knn_idx = knn_idx[self_mask].view(P, -1)

        knn_idx = knn_idx[:, :min(k, P - 1)]

        return knn_idx

    def forward(self, local_feat, patch_center, patch_radius=None):
        """
        local_feat:   (P, C, N)
        patch_center: (P, 3) or (P, 1, 3)
        patch_radius: (P, 1), optional
        """
        P, C, N = local_feat.shape

        if patch_center.dim() == 3:
            patch_center = patch_center.squeeze(-1)   # (P, 3)
        if patch_radius.dim() == 3:
            patch_radius = patch_radius.squeeze(-1)   # (P, 3)

        patch_max = torch.max(local_feat, dim=-1)[0]   # (P, C)
        patch_mean = torch.mean(local_feat, dim=-1)    # (P, C)

        if self.use_radius and patch_radius is not None:
            token_in = torch.cat([patch_max, patch_mean, patch_radius], dim=1)  # (P, 2C+1)
        else:
            token_in = torch.cat([patch_max, patch_mean], dim=1)                # (P, 2C)

        patch_token = self.token_mlp(token_in)   # (P, Cp)

        if P == 1:
            return patch_token, patch_token

        nbr_idx = self._knn_indices(patch_center, self.k)   # (P, k)
        k = nbr_idx.shape[1]

        nbr_token = patch_token[nbr_idx]                    # (P, k, Cp)
        nbr_center = patch_center[nbr_idx]                  # (P, k, 3)

        center_token = patch_token.unsqueeze(1).expand(-1, k, -1)    # (P, k, Cp)
        rel = nbr_center - patch_center.unsqueeze(1)                  # (P, k, 3)
        dist = torch.norm(rel, dim=-1, keepdim=True)                  # (P, k, 1)

        attn_in = torch.cat([center_token, nbr_token, rel, dist], dim=-1)  # (P, k, 2Cp+4)
        attn = self.attn_mlp(attn_in).squeeze(-1)                            # (P, k)
        attn = F.softmax(attn, dim=1)

        nbr_agg = torch.sum(nbr_token * attn.unsqueeze(-1), dim=1)          # (P, Cp)

        patch_context = self.context_mlp(torch.cat([patch_token, nbr_agg], dim=1))  # (P, Cp)

        return patch_token, patch_context
...
class Decoder(nn.Module):
    def __init__(self, cfgs):
        ...
        self.patch_dim = getattr(cfgs, "patch_dim", cfgs.out_dim)
        self.patch_k = getattr(cfgs, "patch_k", 8)
        self.use_patch_radius = getattr(cfgs, "use_patch_radius", True)

        self.patch_context_agg = PatchContextAggregator(
            feat_dim=cfgs.out_dim,
            patch_dim=self.patch_dim,
            k=self.patch_k,
            use_radius=self.use_patch_radius
        )

        self.patch_fuse = nn.Sequential(
            nn.Conv1d(cfgs.out_dim + self.patch_dim, cfgs.out_dim, 1),
            nn.ReLU(inplace=True),
            nn.Conv1d(cfgs.out_dim, cfgs.out_dim, 1),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, pos, feat, patch_center=None, patch_radius=None):
        B, _, N = pos.shape
        ...
        if patch_center is not None:
            patch_token, patch_context = self.patch_context_agg(
                local_feat=local_feat,
                patch_center=patch_center,
                patch_radius=patch_radius
            )   # both: (P, patch_dim)

            patch_context_expand = patch_context.unsqueeze(-1).expand(-1, -1, N)  # (P, patch_dim, N)
            local_feat = self.patch_fuse(torch.cat([local_feat, patch_context_expand], dim=1))
        ...
        disp_feat = qs.view(B, self.disp_dim, -1)  # (P, d, N*r)
        disp_feat = self.skip_mlp(torch.cat([
            disp_feat,
            feat.unsqueeze(-1).repeat(1, 1, 1, self.r).view(B, self.dim, -1)
        ], dim=1))

        disp = torch.tanh(self.disp_mlp(disp_feat))
        new_pos = pos.unsqueeze(-1).repeat(1, 1, 1, self.r).view(B, 3, -1) + disp
